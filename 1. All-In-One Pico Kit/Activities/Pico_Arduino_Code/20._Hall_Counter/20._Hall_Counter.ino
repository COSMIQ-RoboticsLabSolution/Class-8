...
// Pin definitions
const int hall = 21; // Define the pin connected to the Hall effect sensor
const int redLed = 18; // Define the pin connected to the red LED

int count = 0; // Variable to store the counter value

void updateCounterDisplay(int count) {
  gfx.fillScreen(TFT_BLACK); // Clear the screen with black
  gfx.setCursor(60, 100, 4); // Set the cursor position and text size
  gfx.setTextSize(1); // Set the text size
  gfx.setTextColor(TFT_WHITE); // Set the text color to white
  gfx.printf("counter: %d", count); // Display the counter value
}
...
void setup() {
  ...
  pinMode(hall, INPUT); // Set the Hall effect sensor pin as an input
  pinMode(redLed, OUTPUT); // Set the red LED pin as an output

  updateCounterDisplay(count); // Update the display with the initial counter value
}

void loop() {
  delay(10); // Short delay to reduce CPU usage

  if (digitalRead(hall) == LOW) { // Check if the Hall effect sensor is triggered
    delay(50); // Shake // Wait for 50ms to prevent shaking when it first becomes LOW
    if (digitalRead(hall) == LOW) { // Nesting two layers here is to achieve more reliable debounce processing, 
                                    // preventing Hall sensors from misjudging or repeating counting due to level jitter or noise interference when the magnet approaches.
      // Wait for the magnet to move away to prevent multiple counts
      while (digitalRead(hall) == LOW); // The second confirmation is still LOW, indicating a stable trigger signal
                                        // Wait for the magnet to move away to prevent you from counting again before moving the magnet away.
      count++; // Increment the counter
      updateCounterDisplay(count); // Update the display with the new counter value
